...
class CacheController(wiring.Component):
        ...
        super().__init__(
            {
                ...
                "store": Out(CacheStoreSignature(self.config)),
                "directory": Out(DirectorySignature(self.config)),
    ...
    def elaborate(self, platform) -> Module:
        ...
        with m.If(~fe_read_data_select_buffer):
            m.d.comb += self.fe.read_data.eq(self.store.read_data)
        with m.Else():
        ...
        def prepare_block_write_back(next_state: States):
            ...
            The directory read also has to be performed manually.
            ...
            m.d.sync += evict_block_way.eq(self.repl_pol.next_replacement)
                ...
                with m.Elif(self.fe.request_valid):
                    ...
                    for i in range(self.config.num_ways):
                        m.d.comb += hit_vector[i].eq(
                            (self.directory.tag(i) == fe_address_view.tag)
                            & self.directory.valid_bits[i]
                        )
                    ...
                    with m.Else():
                        ...
                        with m.Else():
                            ...
                            write_back_needed = self.config.write_back & self.directory.dirty_bits.bit_select(self.repl_pol.next_replacement, 1)
                            ...
                            with m.If(self.fe.write_strobe.any()):
                                ...
                                with m.Else():
                                    ...
                                    if self.config.write_back:
                                        with m.If(self.directory.dirty_bits.bit_select(self.repl_pol.next_replacement, 1)):
                                            # write back if dirty
                                            ...
                                            m.d.sync += write_back_address.tag.eq(self.directory.tag(self.repl_pol.next_replacement))
                                            m.d.sync += write_back_address.word_offset.eq(0)
                                            m.d.sync += write_back_way.eq(self.repl_pol.next_replacement)
                                            ...
                                            m.d.comb += [
                                                self.store.read_request_valid.eq(1),
                                                self.store.read_address.eq(Cat(C(0, unsigned(self.config.word_offset_width)), fe_address_view.index)),
            ...
            with m.Case(States.WRITE_BACK_BLOCK):
                ...
                with m.If((~(write_back_data_from_buffer & evict_block_enable))):
                    ...
                    with m.Else():
                        ...
                        m.d.comb += be_buffer_write_data.eq(self.store.read_data)
            ...
            with m.Case(States.FLUSH_CACHE):
                ...
                with m.Else():
                    ...
                    with m.If(self.directory.dirty_bits.bit_select(flush_block_index, 1) & self.directory.valid_bits.bit_select(flush_block_index, 1)):
                        ...
                        m.d.sync += write_back_data_from_buffer.eq(1)
                        m.d.sync += write_back_next_state.eq(next_state)
                        m.d.sync += write_back_address.tag.eq(self.directory.tag(flush_block_index))
