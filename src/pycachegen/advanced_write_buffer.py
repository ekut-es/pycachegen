...
class AdvancedWriteBuffer(wiring.Component):
    def __init__(self, *, signature: MemoryBusSignature, depth: int):
        """A more advanced write buffer with configurable depth.
    ...
    def elaborate(self, platform):
        ...
        for idx, el in enumerate(storage):
            ...
            m.d.comb += addr_match_vec[idx].eq(
                ...
                & Mux(read_ptr <= write_ptr, (idx >= read_ptr) & (idx < write_ptr), (idx >= read_ptr) | (idx < write_ptr))
            )
        ...
        fifo_read = Signal()
        fifo_write = Signal()
        req_fully_in_buffer = addr_match_vec.any() & storage[addr_match_idx].write_strobe.all()

        with m.If(fe_read & ~req_fully_in_buffer):
            # Send FE read request directly to BE
            m.d.comb += [
        ...
        with m.Else():
            with m.If(fe_read & req_fully_in_buffer):
                # Read request can be answered by buffer
