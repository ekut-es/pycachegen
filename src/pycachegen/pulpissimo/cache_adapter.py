...
class CacheAdapter(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        with m.If(processing_read):
            ...
            m.d.comb += core_if.r_valid.eq(cache.fe.read_data_valid)
        ...
        with m.If(processing_write):
            ...
            m.d.comb += core_if.r_valid.eq(cache.fe.port_ready)
        ...
        with m.If(core_if.gnt):
            m.d.sync += processing_read.eq(core_if.wen)
            m.d.sync += processing_write.eq(~core_if.wen)
        ...
        m.d.comb += cache.fe.write_strobe.eq(Mux(core_if.wen, 0, core_if.be))
        m.d.comb += cache.fe.address.eq(core_if.add[2:])
        m.d.comb += cache.fe.write_data.eq(core_if.wdata)
        m.d.comb += core_if.r_rdata.eq(cache.fe.read_data)
        m.d.comb += core_if.r_opc.eq(0)  # Errors are not supported by the cache
        ...
        with m.If(read_sent & mem_if.r_valid):
            ...
            m.d.sync += read_data_buffer.eq(mem_if.r_rdata)

        with m.If(mem_if.req & mem_if.gnt):
            ...
            m.d.sync += read_sent.eq(mem_if.wen)

        with m.If(read_sent):
            with m.If(~read_answered):
                # set read_data_valid to rvalid when a read request was
                # sent but not yet answered
                m.d.comb += cache.be.read_data_valid.eq(mem_if.r_valid)
                # also hand out the mem_if r_rdata
                m.d.comb += cache.be.read_data.eq(mem_if.r_rdata)
            with m.Else():
        ...
        m.d.comb += mem_if.wen.eq(~cache.be.write_strobe.any())
        m.d.comb += mem_if.add.eq(Cat(C(0, unsigned(2)), cache.be.address))
