...
class WriteBuffer(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        with m.If(~outstanding_be_read_response | self.be.read_data_valid):
            # Accept new requests when we're not awaiting a response from the BE
            with m.If(fe_read & ~addr_match_vec.any()):
                ...
                with m.If(self.be.port_ready):
                    m.d.sync += outstanding_be_read_response.eq(1)
            with m.Else():
