...
#include "V{{ name }}_FunctionalUnit.h"
...
int sc_main(int argc, char** argv) {
    ...
    const std::unique_ptr<V{{ name }}_FunctionalUnit> functional_unit{new V{{ name }}_FunctionalUnit{"{{ name }}_FunctionalUnit"}};

    functional_unit->clk_i(clk_i);
    functional_unit->reset_n_i(reset_n_i);

    functional_unit->instruction_i(instruction_i);
    functional_unit->instruction_valid_i(instruction_valid_i);
    functional_unit->next_stage_ready_i(next_stage_ready_i);

    functional_unit->instruction_o(instruction_o);
    functional_unit->instruction_valid_o(instruction_valid_o);
    functional_unit->ready_o(ready_o);

    int latency = {{ latency }}; 

    // start simulation and trace
    std::cout << "{{ name }}_FunctionalUnit start!" << std::endl;

    sc_start(0, SC_NS);

    VerilatedVcdSc* trace = new VerilatedVcdSc();
    functional_unit->trace(trace, 99);

    if(vcd_file_path.empty()) {
        trace->open("{{ vcd_dir_path }}/{{ name }}_FunctionalUnit.vcd");
    } else {
    ...
    std::cout << "{{ name }}_FunctionalUnit done!" << std::endl;
    return 0;
}
