...
class MemoryVerilogTemplate(ACADLObjectVerilogTemplate):
    ...
    def __init__(self,
        ...
        for address_range in self.address_ranges:
            self.address_translation_map[address_range] = -address_range[0] + last_address_of_previous_translated_address_range
            last_address_of_previous_translated_address_range = address_range[0] + self.address_translation_map[address_range] + address_range[1] - address_range[0]
    ...
    def generate_module(self) -> Module:
        ...
        m = Module(f"{self.name}_Memory")

        READ_LATENCY = m.Parameter('READ_LATENCY', self.acadl_object.read_latency.int_latency - 1)
        WRITE_LATENCY = m.Parameter('WRITE_LATENCY', self.acadl_object.write_latency.int_latency - 1)
        DATA_WIDTH = m.Parameter('DATA_WIDTH', self.acadl_object.data_width)
        MAX_DATA_WORD_DISTANCE = m.Parameter('MAX_DATA_WORD_DISTANCE', self.max_data_word_distance)
        MAX_CONCURRENT_REQUESTS = m.Parameter('MAX_CONCURRENT_REQUESTS', self.acadl_object.max_concurrent_requests)
        ...
        INTERNAL_ADDRESS_WIDTH = m.Localparam('INTERNAL_ADDRESS_WIDTH', ceil(log2(self.memory_lines)))
        LATENCY_COUNTER_SIZE = m.Localparam('LATENCY_COUNTER_SIZE', ceil(log2(max(self.acadl_object.read_latency.int_latency, self.acadl_object.write_latency.int_latency))) + 1)
        read_latency_counter = m.Reg('read_latency_counter', LATENCY_COUNTER_SIZE)
        write_latency_counter = m.Reg('write_latency_counter', LATENCY_COUNTER_SIZE)

        mem = m.Reg('mem', DATA_WIDTH, self.memory_lines)
        if self.memory_file_path is not None:
            m.Initial(
                Systask('display', f'Loading memory from {self.memory_file_path}.'),
        ...
        m.Assign(ready(Cond(AndList(Not(write_in_progress), Not(read_in_progress)), 1, 0)))
        ...
        m.Always(Posedge(clk_i), Negedge(reset_n_i))(
            If(reset_n_i == 0)(
                ...
                If(ready == 1)(
                    ...
                    If(AndList(read_write_select == 0,
                                address_valid_i == 1))(
                        read_in_progress(1),
                    ...
                    # initialize write
                    If(AndList(read_write_select == 1,
                                    address_valid_i == 1,
                                    write_data_valid == 1))(
                        write_in_progress(1),
    ...
    def generate_test_bench(self,
        ...
        # generate verilog
        self.generate_module().to_verilog(filename = target_dir_path + f"/{self.name}_{self.memory_verilog_file_name}", for_verilator = True)
        ...
        read_write_template(
            ...
            name=self.name,
            read_latency=self.acadl_object.read_latency.int_latency,
            write_latency=self.acadl_object.write_latency.int_latency,
            address_ranges=self.address_ranges,)

        # generate CMakeLists.txt
        read_write_template(self.memory_template_dir_path +
                            "/CMakeLists.txt",
