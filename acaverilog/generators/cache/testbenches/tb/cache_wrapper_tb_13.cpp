...
// L1: data_width=16, num_ways=2, num_sets=2,
// replacement_policy=plru_tree, hit_latency=4, miss_latency=29,
// write_through=false, write_allocate=true, block_size=4
...
int sc_main(int argc, char** argv) {
    ...
    const std::unique_ptr<Vcache_wrapper_13> cache_wrapper{
        new Vcache_wrapper_13{"cache_wrapper"}};
    ...
    std::cout << "Vcache_wrapper_13 start!" << std::endl;
    ...
    if (vcd_file_path.empty()) {
        trace->open("Vcache_wrapper_tb_13.vcd");
    } else {
    ...
    std::cout << "Vcache_wrapper_13 done!" << std::endl;
    return 0;
}