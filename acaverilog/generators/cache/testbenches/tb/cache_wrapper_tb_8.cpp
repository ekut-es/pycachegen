...
// write_through=false, write_allocate=false, block_size=1
...
int sc_main(int argc, char** argv) {
    ...
    const std::unique_ptr<Vcache_wrapper_8> cache_wrapper{
        new Vcache_wrapper_8{"cache_wrapper"}};
   ...
   auto tick = [&](int amount) {
    ...
    std::cout << "Vcache_wrapper_8 start!" << std::endl;
    ...
    if (vcd_file_path.empty()) {
        trace->open("Vcache_wrapper_tb_8.vcd");
    } else {
    ...
    std::cout << "Vcache_wrapper_8 done!" << std::endl;
    return 0;
}