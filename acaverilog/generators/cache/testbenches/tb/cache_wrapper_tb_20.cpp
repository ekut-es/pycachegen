...
// L2: data_width=64, num_ways=1, num_sets=2,
// replacement_policy=plru_tree, hit_latency=4, miss_latency=11,
// write_through=false, write_allocate=true, block_size=1
// Main Memory: data_width=128, read_latency=10, write_latency=15
...
int sc_main(int argc, char** argv) {
    ...
    std::cout << "Vcache_wrapper_20 start!" << std::endl;
    ...
    if (vcd_file_path.empty()) {
        trace->open("Vcache_wrapper_tb_20.vcd");
    } else {
    ...
    std::cout << "Vcache_wrapper_20 done!" << std::endl;
    return exit_code;
}