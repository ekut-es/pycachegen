...
#include "Vcache_wrapper_29.h"

// Testbench for testing flushes in multi layer cache architectures
// num_ports=1, arbiter_policy=priority, byte_size=8
// L1: data_width=8, address_width=3, num_ways=1, num_sets=2,
// replacement_policy=plru_tree, hit_latency=0, miss_latency=0,
// write_through=false, write_allocate=true,
// block_size=1
// L2: data_width=8, address_width=3, num_ways=1, num_sets=2,
// replacement_policy=plru_tree, hit_latency=0, miss_latency=0,
// write_through=true, write_allocate=false,
// block_size=1
// L3: data_width=8, address_width=3, num_ways=1, num_sets=2,
// replacement_policy=plru_tree, hit_latency=0, miss_latency=0,
// write_through=false, write_allocate=true,
// block_size=1
// Main Memory: data_width=8, address_width=3, read_latency=3, write_latency=5
...
int sc_main(int argc, char** argv) {
    ...
    const std::unique_ptr<Vcache_wrapper_29> cache_wrapper{
        new Vcache_wrapper_29{"cache_wrapper"}};
    ...
    std::cout << "Vcache_wrapper_29 start!" << std::endl;
    ...
    if (vcd_file_path.empty()) {
        trace->open("Vcache_wrapper_tb_29.vcd");
    } else {
    ...
    std::cout << "Vcache_wrapper_29 done!" << std::endl;
    return 0;
}