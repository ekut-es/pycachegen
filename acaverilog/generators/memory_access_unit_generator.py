...
class MemoryAccessUnitGenerator(FunctionalUnitGenerator):
    ...
    def generate_module(self) -> Module:
        ...
        OPCODE_END_BIT = m.Parameter('OPCODE_END_BIT', self.opcode_end_bit)
        OPCODE_LENGTH = m.Parameter('OPCODE_LENGTH', self.opcode_length)
        REGISTER_LENGTH = m.Parameter('REGISTER_LENGTH', 1)  # Dummy value
        ADDRESS_LENGTH = m.Parameter('ADDRESS_LENGTH', 1)  # Dummy value
        ...
        register_read_i = m.Input('read_i', DATA_WIDTH)
        register_write_done_i = m.Input('write_done_i')
        memory_ready_i = m.Input('memory_ready_i')
        memory_read_i = m.Input('memory_read_i', DATA_WIDTH)
