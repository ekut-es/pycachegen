...
int sc_main(int argc, char** argv) {
    ...
    const int parameter_num_registers =
        VRegisterFile_RegisterFile::NUM_REGISTERS;
    const int parameter_read_write_ports =
        VRegisterFile_RegisterFile::READ_WRITE_PORTS;

    std::cout << "DATA_WIDTH:       " << parameter_data_width << std::endl;
    std::cout << "NUM_REGISTERS:    " << parameter_num_registers << std::endl;
    std::cout << "READ_WRITE_PORTS: " << parameter_read_write_ports
              << std::endl;
    ...
    // sc_signal<sc_bv<parameter_data_width*parameter_read_write_ports>>
    // data_i;
    ...
    for (int j = 0; j < parameter_read_write_ports; j++) {
        ...
        for (int i = 0; i < parameter_num_registers; i++) {
            ...
            assert(output_data.to_uint() ==
                   value_assignment_map[i + parameter_num_registers * j]);
        }
    }
