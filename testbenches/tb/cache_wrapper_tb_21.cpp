...
// Main Memory: data_width=64, read_latency=10, write_latency=15
// min_address=0, max_address=64
...
int sc_main(int argc, char** argv) {
    ...
    const std::unique_ptr<Vcache_wrapper_21> cache_wrapper{
        new Vcache_wrapper_21{"cache_wrapper"}};
    ...
    std::cout << "Vcache_wrapper_21 start!" << std::endl;
    ...
    if (vcd_file_path.empty()) {
        trace->open("Vcache_wrapper_tb_21.vcd");
    } else {
    ...
    std::cout << "Vcache_wrapper_21 done!" << std::endl;
    return exit_code;
}